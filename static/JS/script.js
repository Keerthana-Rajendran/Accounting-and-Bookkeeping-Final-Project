...
const re_acc_num =document.querySelector('#Re_Acc_Number')
...
function validateForm(){
    ...
    }else if(acc_num.value.trim().length > 11 || acc_num.value.trim().length < 11 || isNaN(acc_num.value)){
        setError(acc_num, "Invalid Account Number");
    }else{
        setSuccess(acc_num);
    }if (re_acc_num.value.trim() == ''){
        setError(re_acc_num,"Re Enter Account Number");
    }else if(re_acc_num.value !== acc_num.value){
        setError(re_acc_num, "Account Number Does not Match");
    }else{
        setSuccess(re_acc_num);
