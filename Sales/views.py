...
from Utils.utils import isNoneOrEmpty
...
class SalesView(ListView):
    ...
    def get_queryset(self):
        ...
        if isNoneOrEmpty(filter_val):
            new_context = SalesBill.objects.all().filter()
        else:
            new_context = SalesBill.objects.filter(billno=filter_val)
            print(filter_val)
            print(new_context)
        return new_context
